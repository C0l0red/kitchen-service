...
export default class MenuItemsController {
    ...
    async listMenuItemsForVendor(request: Request, response: Response, next: NextFunction) {
        try {
            ...
            const data = await this.menuItemsService.listMenuItemsForVendor(vendorId, pagedRequest);
            const responseData = buildResponse('Menu Items fetched successfully', data);
